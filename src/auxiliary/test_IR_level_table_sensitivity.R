...
for(s in 1:3){
    res <- readRDS(file.path("models", paste0("test_IR_level_sensitivity_",s,".RDS")))
    ...
    tab_wb$years <- NULL
    write.csv(tab_asy, file.path("outputs", "tables", paste0("test_IR_level_table_asy_sensitivity_",s,".csv")), row.names = FALSE)
    write.csv(tab_wb, file.path("outputs", "tables", paste0("test_IR_level_table_wb_sensitivity_",s,".csv")), row.names = FALSE)
}
