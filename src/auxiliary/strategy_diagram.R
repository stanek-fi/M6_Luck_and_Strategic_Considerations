...
for (q in qs) {
    ...
    temp[, position := factor(position, levels = c("short", "long"), labels = c(TeX("$\\beta^{-}_{m,k}$"), TeX("$\\beta^{+}_{m,k}$")))]
    p <- ggplot(temp, aes(x = "", y = value, fill = position)) +
        geom_bar(width = 1, stat = "identity") +
        coord_polar("y", start = 0) +
        facet_grid(delta_state ~ t, switch = "both") +
        theme(axis.text.x = element_blank(), axis.ticks = element_blank(), legend.position = "bottom", legend.title = element_blank()) +
