...
for (s in 1:3){
    ...
    participant_rank <- readRDS(file.path("models", paste0("participant_rank_simulated_sensitivity_",s,".RDS")))
    participant_state <- readRDS(file.path("models", paste0("participant_state_simulated_sensitivity_",s,".RDS")))
    participant_beta <- readRDS(file.path("models", paste0("participant_beta_simulated_sensitivity_",s,".RDS")))
    ...
    ggsave(file.path("outputs", "plots", paste0("rank_histogram_simulated_sensitivity_",s,".png")), rank_histogram, width = 6.6, height = 6)
    ...
    temp$beta <- NULL
    write.csv(temp, file.path("outputs", "tables", paste0("state_table_simulated_tangency_sensitivity_",s,".csv")), row.names = FALSE)
    ...
    write.csv(temp, file.path("outputs", "tables", paste0("state_table_simulated_rank_optimization_sensitivity_",s,".csv")), row.names = FALSE)
