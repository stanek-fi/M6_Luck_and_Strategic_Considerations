...
statistics <- function(x) {
    list(
        mean = mean(x),
        sd = sd(x),
        q_01 = quantile(x, 0.01),
        q_99 = quantile(x, 0.99)
    )
}
...
temp_12 <- na.omit(do.call(rbind,lapply(1:12, function(i) {
    temp <- compute_round(returns_list[[i]], positions_list[[i]])
    data.table(
        id=names(temp),
        ir=temp,
        month=i
...
temp_global <- compute_round(returns_list, positions_list)
temp_global <- data.table(
        id=names(temp_global),
        ir=temp_global,
        month="global"
)
leaderboard_computed <- rbind(temp_12, temp_global)
statistics_realized <- leaderboard_computed[, statistics(ir), month]
...
statistics_simulated <- lapply(1:par$rep_leaderboard_comparison, function(r) {
    ...
    positions_list <- lapply(1:par$rounds_num_fixed, function(i) {
        position_baseline(
    ...
    temp_12 <- do.call(rbind, lapply(1:par$rounds_num_fixed, function(i) {
        compute_round(returns_list[[i]], positions_list[[i]])
    }))
    temp_global <- compute_round(returns_list, positions_list)
    temp <- rbind(temp_12, temp_global)
    temp <- rbindlist(apply(temp, 1, statistics))
    temp[, month := c(1:par$rounds_num_fixed, "global")]
    temp
})
