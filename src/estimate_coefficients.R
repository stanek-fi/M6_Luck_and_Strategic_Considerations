...
out <- rbindlist(lapply(seq_len(nrow(combinations)), function(i) {
    ...
    temp <- rbindlist(lapply(1:par$rounds_num_fixed, function(m) {
        returns <- returns_list[[m]]

        # temp <- rbindlist(lapply(1:par$rep_estimate_coefficients, function(r) { # * single-thread
        #     set.seed(r)
        ...
        clusterExport(cl, c("par", "num_shorted", "num_zeros", "compute_round", "position_baseline"), envir = parent.env(environment())) # * multi-thread
        temp <- rbindlist(foreach(r = 1:par$rep_estimate_coefficients) %dopar% {
            set.seed(r)
            positions <- position_baseline(par$returns_num_fixed, par$participants_num_fixed, num_shorted, num_zeros)
            states <- compute_round(returns, positions)
            list(
