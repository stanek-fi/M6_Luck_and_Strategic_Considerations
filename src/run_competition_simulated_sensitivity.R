...
for(s in seq_len(nrow(sensitivity_parameters))){
    ...
    res <- foreach(r = 1:par$rep_simulate_competition) %dopar% { # * multi-thread
        ...
        returns <- simulate_returns_BEKK(par$rounds_num, par$days_per_round, mu, sigma, lambda = returns_lambda, alpha = alpha, beta = beta, df = df, burnin = 10)
    ...
    saveRDS(participant_rank, file.path("models", paste0("participant_rank_simulated_sensitivity_", s, ".RDS")))
    saveRDS(participant_state, file.path("models", paste0("participant_state_simulated_sensitivity_", s, ".RDS")))
    saveRDS(participant_beta, file.path("models", paste0("participant_beta_simulated_sensitivity_", s, ".RDS")))
